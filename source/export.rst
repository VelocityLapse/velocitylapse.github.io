...
Select either the ``MP4 video`` or ``Image Sequence`` option to set the export format.
...
The output location where the JPG image sequence will be saved. This can be changed by setting the folder of the ``Image Export Save Location`` setting.
